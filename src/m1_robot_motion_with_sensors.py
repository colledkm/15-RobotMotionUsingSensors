...
def run_test_go_straight_for_seconds():
    ...
    robot.go_straight_for_seconds(2,0)
...
def run_test_go_straight_for_inches():
    ...
    robot = SimpleRoseBot()
    robot.go_straight_for_inches(3, 20)
    robot.go_straight_for_inches(2, 70)
    robot.go_straight_for_inches(5, 50)
...
class SimpleRoseBot(object):
    ...
    def go_straight_for_inches(self,inches,speed):
        self.go(speed,speed)
        start = (self.left_wheel_motor.get_position()/360)*2*math.pi*1.3*0.5
        while True:
            current = (self.left_wheel_motor.get_position()/360)*2*math.pi*1.3*0.5
            if current-start > inches:
                break
        self.stop()
    ...
    def go_straight_until_black(self,speed):
        self.go(speed,speed)
        while True:
            color = ColorSensor.get_reflected_light_intensity()
            if color == 1:
                break
        self.stop()
